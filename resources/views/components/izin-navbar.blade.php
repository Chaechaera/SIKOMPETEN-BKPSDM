...
<header x-data="{ scrolled: false }" @scroll.window="scrolled = window.scrollY > 20" :class="scrolled
    ...
    class="fixed top-0 left-0 w-full px-8 py-1 flex justify-between items-center
    ...
    <!-- Navigation -->
    <nav class="hidden md:flex items-center gap-10 font-semibold text-sm">
        @auth
        ...
        <div x-data="{
            ...
            <div @click="openProfile = !openProfile" class="flex items-center gap-2 cursor-pointer select-none">
                ...
                <i data-lucide="chevron-down" class="text-sm transition-transform text-white duration-200" :class="{ 'rotate-180': openProfile }"></i>
            </div>

            <!-- Dropdown Profile -->
            <div x-show="openProfile" @click.outside="openProfile = false" x-transition
                class="absolute right-0 mt-3 w-64 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden text-gray-800">
                ...
                <div class="flex items-center space-x-4 p-4 border-b border-abuabuCerah">
                    <div class="flex flex-col">
                        ...
                        <div class="flex items-center gap-2 text-sm py-1">
                            ...
                            <span>{{ Auth::user()->subunitkerjas->sub_unitkerja ?? '-' }}</span>
                        </div>
                        <div class="flex items-center gap-2 text-sm py-1">
                ...
                <!-- Menu -->
                <a href="{{ route('profile.edit') }}" class="block px-4 py-3 hover:bg-gray-300 text-sm">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full text-left px-4 py-3 hover:bg-gray-300 text-sm">
                        <div class="flex items-center gap-2">
